...
const MODEL_PRESETS = [
  { name: "Sports Bike", url: "/Karandeep.glb", iosUrl: "/bama33.usdz", dimensions: { length: "2.1m", width: "0.8m", height: "1.2m" } },
  { name: "Sports Car", url: "/mod1.glb", iosUrl: "/bama33.usdz", dimensions: { length: "4.5m", width: "1.9m", height: "1.3m" } },
  { name: "Luxury Sedan", url: "/mod3.glb", iosUrl: "/bama33.usdz", dimensions: { length: "5.1m", width: "1.9m", height: "1.5m" } },
  { name: "sphere", url: "/bamaa33.glb", iosUrl: "/bamaa33.usdz", dimensions: { length: "5.1m", width: "1.9m", height: "1.5m" } },
];
...
function ARModelViewer({ modelUrl, modelIndex, selectedModelColor })
{
  ...
  // Get the iOS-specific USDZ file path from the MODEL_PRESETS
  const iosModelPath = MODEL_PRESETS[modelIndex].iosUrl;
  ...
  useEffect(() => {
    ...
    // For iOS, we need to ensure the model-viewer is properly initialized
    // if (iOS && modelViewerRef.current) {
    //   // Force a redraw of the model-viewer component
    //   const modelViewer = modelViewerRef.current;
    //   setTimeout(() => {
    //     if (modelViewer) {
  ...
  const getIOSARLink = () => {
    // For iOS, we need to create a direct AR Quick Look link
    const fullIOSUrl = `${window.location.origin}${iosModelPath}`;
    // Format as a special iOS AR Quick Look URL
    return `intent://arvr.google.com/scene-viewer/1.0?file=${encodeURIComponent(fullIOSUrl)}&mode=ar_preferred#Intent;scheme=https;package=com.google.android.googlequicksearchbox;action=android.intent.action.VIEW;S.browser_fallback_url=${encodeURIComponent(window.location.href)};end;`;
  };
    ...
    <div style={{ textAlign: "center", padding: "20px" }}>
      ...
      <div style={{ maxWidth: "500px", margin: "0 auto" }}>
        {/* {!isIOS &&<model-viewer
          ref={modelViewerRef}
          src={modelUrl}
          ios-src={window.location.origin + iosModelPath} // Ensure full URL for iOS
          alt={`3D model of ${MODEL_PRESETS[modelIndex].name}`}
          ar
          ar-modes="webxr scene-viewer quick-look"
          camera-controls
...
{isIOS && (
  <div style={{
    ...
    <a 
      rel="ar" 
      href={`${window.location.origin}${iosModelPath}`}
      style={{
    ...
    >
      <svg width="20" height="20" style={{ marginRight: "8px" }} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      View in AR Quick Look
        ...
        <div style={{ marginTop: "20px", fontSize: "14px" }}>
          <p>Scan this QR code to open AR view on another device:</p>
          <div style={{ display: "flex", justifyContent: "center" }}>
            <QRCodeCanvas value={window.location.origin + modelUrl + "?ar=true&model=" + modelIndex} size={150} />
          </div>
        </div>
        ...
        {isIOS && (
          <div style={{ 
            ...
            <p><strong>iOS Users:</strong> If the AR view doesn't load, try these steps:</p>
            <ol style={{ textAlign: "left", paddingLeft: "20px" }}>
              <li>Make sure you're using Safari browser</li>
              <li>Check that your device supports AR Quick Look (iOS 12+)</li>
              <li>Ensure you have allowed camera access</li>
            </ol>
          </div>
        )}
      </div>
...
export default function VehicleShowcase() {
  ...
  //   return `${window.location.origin}${window.location.pathname}?model=${modelIndex}&color=${modelColorIndex}&ar=true`;
  // };
  // In the VehicleShowcase component, update the getShareableARLink function:
...
const getShareableARLink = () => {
  ...
  if (iOS) {
    // For iOS, link directly to the USDZ file with special AR Quick Look URL
    const iosUrl = `${window.location.origin}${MODEL_PRESETS[modelIndex].iosUrl}`;
    return iosUrl;
  } else {
    // Standard web AR link for Android and other platforms
    return `${baseUrl}?model=${modelIndex}&color=${modelColorIndex}&ar=true`;
  }
};

// Update the useEffect hook in VehicleShowcase to better handle AR mode:
useEffect(() => {
  const params = new URLSearchParams(window.location.search);
  ...
  if (params.get('ar') === 'true') {
    ...
    if (iOS) {
      ...
      // We'll redirect to the AR Quick Look URL directly if this is the first load
      const hasRedirected = sessionStorage.getItem('ar_redirected');
      
      if (!hasRedirected) {
        // Set a flag to prevent redirect loops
        sessionStorage.setItem('ar_redirected', 'true');
        
        // Get the iOS USDZ file path
        const iosUrl = `${window.location.origin}${MODEL_PRESETS[modelIndex].iosUrl}`;
        
        // Redirect after a small delay to ensure state is set
        setTimeout(() => {
          window.location.href = iosUrl;
        }, 100);
        
        return; // Early return to prevent AR viewer from showing before redirect
      }
    }
  ...
  if (showARViewer) {
    return (
      <div style={{ height: "100vh", width: "100%", position: "relative" }}>
        <ARModelViewer 
          modelUrl={window.location.origin + selectedModelUrl} 
          modelIndex={modelIndex}
          selectedModelColor={selectedModelColor} 
        />
        <div style={{ position: "absolute", top: "10px", left: "10px" }}>
